...
class Solution {
    public double champagneTower(int poured, int query_row, int query_glass) {
		//思路：模拟香槟流向的过程，从上往下依次将香槟倒入下一层的杯子中。
		double []row = {poured};
		for(int i = 1; i <= query_row; i++){
			//下一层的所有杯子
			double []next_Row = new double[i+1];
			for(int j = 0; j < i; j++) {
				//往每一个杯子中倒入香槟
				//上一层的第i个杯子中的香槟，流到了下一层的i, i+1个杯子中
				double volumn = row[j];
				if( volumn > 1) {
			...
			//将poured瓶香槟已全部流到第i层，接下来更新香槟塔中杯子的状态
			row = next_Row;
			//再次循环，更新第i+1层的香槟塔状态
		}
