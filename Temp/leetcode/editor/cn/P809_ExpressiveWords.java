...
// 
//
// Related Topics 数组 双指针 字符串 👍 69 👎 0
...
class Solution {
	...
	public boolean wordStretchy(String s, String word) {
		int i = 0, j = 0;
		while(i < s.length() && j < word.length()) {
			int cnti = 0;
			int si = s.charAt(i);
			int cntj = 0;
			int sj = word.charAt(j);
			if(si != sj) {
				return false;
			}
			while(i < s.length() && si == s.charAt(i)) {
				i++;
				cnti++;
			}


			while (j < word.length() && sj == word.charAt(j)) {
				j++;
				cntj++;
			}
			if(cnti < cntj) {
